...
QStringList ChineseLetterHelper::readPYFile(const QString& fileName)
{
    QStringList listPY;
    //从配置文件读取拼音数组
    QFile file("./ChineseToPinyin.txt");
...
QString ChineseLetterHelper::Ch2PY(const QString& src, QStringList listPY)
{
    ...
    for (int i = 0; i < src.length(); i++)
    {
        int unicode = QString::number(src.at(i).unicode(), 10).toInt();	//将每个汉字转换为十进制的unicode编码

        if (unicode >= 0x4E00 && unicode <= 0x9FA5)			//如果是中文则转换
        {
...
QString ChineseLetterHelper::Ch2APY(const QString& ch)
{
    QStringList listJP;
    //加载简拼数组
    ...
    {
        //若是字母或数字则直接输出
        ushort vChar = strChinese.at(i).unicode();
        if ((vChar >= 'a' && vChar <= 'z') || (vChar >= 'A' && vChar <= 'Z'))
        {
            str.append(strChinese.at(i).toUpper());
        }
        if ((vChar >= '0' && vChar <= '9'))
        {
            str.append(strChinese.at(i));
        }
        else
        {
